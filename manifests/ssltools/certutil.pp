...
define kcerts::ssltools::certutil($nss_db_dir, $client_cert, $cert_name=$title, $refreshonly = true, $trustargs = ',,') {
  ...
  exec { $cert_name:
    path        => ['/bin', '/usr/bin'],
    command     => "certutil -A -d '${nss_db_dir}' -n '${cert_name}' -t '${trustargs}' -a -i '${client_cert}' -f '${::kcerts::ssltools::nssdb::nss_db_password_file}'",
